...
import { slugify, today, tokenize, withLock, writeFileAtomic } from "./util.mjs";
...
export function addPolicy({ title, body, scope = "", kind = "policy", triggers = [], source = "user" }) {
    ...
    writeFileAtomic(file, fm.serialize(data, body || title));
    rebuildIndex();
    return file;
}
...
export function addFriction({ title, body }) {
    ...
    writeFileAtomic(file, fm.serialize({ id, title, created: today() }, body || title));
    return file;
}

// Regenerate index.md as the human-readable map of all policies. Locked like
// the other derived indexes so parallel adds converge complete.
export function rebuildIndex() {
    ensureDirs();
    return withLock("memory-index", () => {
